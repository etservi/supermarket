...
public class ArticleVenduController implements Initializable{
...
//		selectionMutiple();
	...
	ObservableList<Article> articleList = FXCollections.observableArrayList();
		...
		public void ActualiserDonneesQrticleDetailsTableau() {
			...
			String requetteIni = "SELECT `nomArticleNom`, `idRayon`, `idCategoriee`, `raisonSociale`, `codeBarre`, qteStock, prixUnitaire, prixAvendre  FROM `Article` WHERE Livrer0nonLivrer1=0"; 
			...
			try {
				...
				Logger.getLogger(ArticleVenduController.class.getName()).log(Level.SEVERE, null, exActualiserDonneesFournisseurTableau);
			}
		}
	...
	//----------------------------------------------------------------------------
		...
		public void listerParDate() {
			...
			String requetteI = "SELECT `nomArticleNom`, `idRayon`, `idCategoriee`, `raisonSociale`, `codeBarre`, qteStock, prixUnitaire, prixAvendre  FROM `Article` WHERE dateAjoutee BETWEEN '" + dateA.getValue()+"' AND '"+ dateB.getValue()+"' AND Livrer0nonLivrer1=0  "; 
