...
public class ArticleEnStockController implements Initializable{
...
//		selectionMutiple();
	...
	ObservableList<Article> articleList = FXCollections.observableArrayList();
		...
		public void ActualiserDonneesQrticleDetailsTableau() {
			...
			try {
				...
				Logger.getLogger(ArticleEnStockController.class.getName()).log(Level.SEVERE, null, exActualiserDonneesFournisseurTableau);
			}
		}
	...
	//----------------------------------------------------------------------------
		...
		public void listerParDate() {
			...
			String requetteI = "SELECT `nomArticleNom`, `idRayon`, `idCategoriee`, `raisonSociale`, `codeBarre`, qteStock, prixUnitaire, prixAvendre  FROM `Article` WHERE dateAjoutee BETWEEN '" + dateA.getValue()+"' AND '"+ dateB.getValue()+"'   "; 
